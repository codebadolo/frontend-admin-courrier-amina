...
import {
  ...
  Tooltip, Divider, Steps, Progress, Typography
} from "antd";
import {
  PlusOutlined,
  ReloadOutlined,
...
const CourrierEntrants = () => {
  ...
  // ============================================
  // GESTION TRADITIONNELLE (sans IA)
  // ============================================
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v) => {
        ...
        return <Tag color={colors[v] || "blue"}>{v || "normale"}</Tag>;
      },
    },
    {
      title: "Date réception",
      dataIndex: "date_reception",
      render: (v) => <span>{v || "Non spécifiée"}</span>,
    },
    {
      title: "Actions",
      key: "actions",
        ...
        <Space size="middle">
          <Tooltip title="Voir les détails">
            <EyeOutlined
              ...
              onClick={() => navigate(`/courriers-entrants/${record.id}`)}
            />
          </Tooltip>
      ...
      <Form form={form} layout="vertical">
        <Row gutter={16}>
          <Col span={12}>
            ...
            >
              <Input placeholder="Objet détecté par l'IA" suffix={<Tag color="blue">IA</Tag>} />
            </Form.Item>
            <Form.Item
              name="expediteur_nom"
              label="Expéditeur"
          ...
          <Col span={12}>
            ...
            <Row gutter={8}>
              ...
              <Col span={12}>
                <Form.Item name="category" label="Catégorie" rules={[{ required: true }]}>
                  <Select placeholder="Catégorie suggérée" loading={categories.length === 0}>
                    {categories.map((c) => (
                      <Option key={c.id} value={c.id}>
                        {c.nom}
                        {aiResult?.category === c.id && (
            ...
            <Form.Item name="service_id" label="Service imputé" rules={[{ required: true }]}>
              <Select placeholder="Service suggéré" loading={services.length === 0}>
                {services.map((s) => (
                  <Option key={s.id} value={s.id}>
                    {s.nom}
                    {aiResult?.service_id === s.id && (
                      <Tag color="green" style={{ marginLeft: 8 }}>
                        IA
                      </Tag>
                    )}
  ...
  return (
    <Card
      ...
      extra={
        <Space>
          <Button icon={<ReloadOutlined />} onClick={loadCourriers} disabled={loading}>
            Actualiser
          </Button>
          <Button
            icon={<PlusOutlined />}
          ...
          <Button type="primary" icon={<RobotOutlined />} onClick={startAiWorkflow}>
            Nouveau avec IA
          </Button>
        </Space>
      }
      ...
      >
        ...
        >
          ...
          <Form.Item style={{ textAlign: "right" }}>
            <Space>
              <Button onClick={() => setFormModalVisible(false)}>Annuler</Button>
      ...
      >
        ...
        {aiWorkflowStep === 1 && (
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 30 }}>
              <Button onClick={() => setAiWorkflowStep(0)}>← Retour à l'upload</Button>
            </div>
          </div>
        )}
        ...
        {aiWorkflowStep === 2 && (
          <div>
            ...
            >
              <div>
                <Button onClick={() => setAiWorkflowStep(0)}>Recommencer</Button>
                <Button
                  onClick={() => setAiWorkflowStep(1)}
                  icon={<EditOutlined />}
                  style={{ marginLeft: 10 }}
